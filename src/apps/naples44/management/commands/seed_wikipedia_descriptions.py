"""
Seed `description` with a 2–3 sentence factual blurb from Wikipedia, for any of
Place / Person / FolkloreEntity that has a `wikipedia_url`.

Only rows whose description is empty are touched, unless --refresh is given.
`book_role` is never touched here — that field is hand-written.

    ./src/manage.py seed_wikipedia_descriptions
    ./src/manage.py seed_wikipedia_descriptions --model folklore --refresh
    ./src/manage.py seed_wikipedia_descriptions --model person --only vincente-lattarullo
"""
...
MODELS = {
    ...
    "folklore": models.FolkloreEntity,
}
...
class Command(BaseCommand):
    help = "Fill description from Wikipedia for Place / Person / FolkloreEntity."

    def add_arguments(self, parser):
        parser.add_argument("--model", choices=sorted(MODELS), action="append",
                            help="Limit to this model (repeatable). Default: all three.")
    ...
    def handle(self, *args, **opts):
        names = opts["model"] or list(MODELS)
        done = skipped = failed = 0

        for name in names:
            Model = MODELS[name]
            qs = Model.objects.exclude(wikipedia_url="").order_by("name")
            if opts["only"]:
            ...
            for obj in qs:
                try:
                    extract = wikipedia_summary(obj.wikipedia_url)
                except Exception as exc:  # noqa: BLE001
                    failed += 1
                    self.stderr.write(f"  {obj.name}: {exc}")
                    continue
                time.sleep(0.2)
                ...
                if not blurb:
                    skipped += 1
                    self.stdout.write(self.style.WARNING(f"  no usable extract: {obj.name}"))
                    continue

                self.stdout.write(f"  {obj.name}: {blurb[:110]}{'…' if len(blurb) > 110 else ''}")
                if not opts["dry_run"]:
                    obj.description = blurb
